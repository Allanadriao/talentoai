...
import { Menu, Search, Bell, Plus, Users } from 'lucide-react';
  ...
  return (
    <header className="h-20 bg-white border-b border-slate-100 flex items-center justify-between px-4 lg:px-8 sticky top-0 z-20">
      ...
      <div className="flex items-center gap-4 lg:gap-6">
        {isAdmin && recruitersList.length > 0 && (
          <div className="hidden lg:flex items-center gap-2 relative">
            <Users size={16} className="text-indigo-400 absolute left-3" />
            <select
              value={selectedRecruiter}
              onChange={(e) => setSelectedRecruiter(e.target.value)}
              className="appearance-none bg-indigo-50/50 border border-indigo-100 text-indigo-700 text-sm rounded-xl py-2 pl-9 pr-8 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 font-bold cursor-pointer hover:bg-indigo-50 transition-colors"
            >
              <option value="all">Todas as Recrutadoras</option>
              {recruitersList.map(email => (
        ...
        <div className="flex items-center gap-3 pl-6 border-l border-slate-100">
          <div className="hidden sm:block text-right">
            <p className="text-sm font-bold">{isAdmin ? 'Super Admin' : 'Admin'}</p>
